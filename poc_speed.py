...
class FullPipelineInferenceBenchmark:
    def __init__(
        self,
    ...
    def infer_with_routing(self, image_path: str, ground_truth: int):
        ...
        if small_result is None:
            ...
            return None
        small_pred, small_prob, small_time = small_result
        ...
        if small_prob < 0.96:
            ...
            if big_result is None:
                ...
                return None
            big_time = big_result[2]
            if not self._is_prediction_belongs_to_global_dataset(big_result[0]):
                return ground_truth, self.not_belong_to_global_idx, 1, big_time + small_time
            ...
            return ground_truth, global_pred, 1, big_time + small_time
        else:
            global_pred = self._translate_prediction_to_global_label(small_pred, ModelType.SMALL_MODEL)
            return ground_truth, global_pred, 0, small_time
        ...
        for run in range(num_runs):
            ...
            for species_id in tqdm(sampled_species, desc=f"Run {run + 1}/{num_runs}", leave=False):
                ...
                if result is not None:
                    ground_truth, pred, did_small_call_big, infer_time = result
                    other_preds += did_small_call_big
                    y_true.append(ground_truth)
                    y_pred.append(pred)
                    all_elapsed_times.append(infer_time)
...
if __name__ == "__main__":
    ...
    pipeline = FullPipelineInferenceBenchmark(
        "models/mcunet_q8.onnx",
        "models/mobilenet_v3_large_100_optimized.onnx",
        global_image_data,
